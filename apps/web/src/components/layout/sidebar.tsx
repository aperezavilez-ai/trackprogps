...
import {
  ...
  Settings, CreditCard, Bot, Building2, ChevronLeft, ChevronRight, Clock
} from 'lucide-react'
...
const NAV_ITEMS = [
  ...
  { href: '/drivers',      label: 'Choferes',      icon: Users,            section: 'fleet' },
    ...
    <aside className={cn(
      ...
      <div className={cn(
        ...
        {!collapsed && (
          <div className="min-w-0">
            <div className="text-sm font-bold truncate">TrackPro</div>
            <div className="text-xs text-gray-400 truncate">{profile.company?.name ?? 'GPS'}</div>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2">
        ...
        {(features['ai_assistant'] || profile.role === 'super_admin') && (
          <div className={cn(
            'mx-2 mt-2 p-3 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl cursor-pointer hover:from-blue-500 hover:to-purple-500 transition',
      ...
      <button
        ...
        className="flex items-center justify-center h-10 border-t border-gray-800 text-gray-500 hover:text-white transition"
      >
