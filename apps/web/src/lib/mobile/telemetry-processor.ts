...
import { batchUpsertPositions } from '@/lib/positions/batch-ingest'
...
export async function processMobileTelemetry(
  ...
  if (validPoints.length > 0) {
    try {
      await batchUpsertPositions(supabase, validPoints.map(v => v.position))
      for (const { pt, position } of validPoints) {
